...
exports.updateStore = exports.deleteStore = exports.deleteUser = exports.getAvailableOwners = exports.getUsers = exports.getStores = exports.addStore = exports.addUser = exports.getStats = void 0;
const bcryptjs_1 = __importDefault(require("bcryptjs"));
const db_1 = require("../config/db");
const getStats = async (req, res) => {
    try {
        const userCountRes = await (0, db_1.query)('SELECT COUNT(*) as count FROM users');
        const storeCountRes = await (0, db_1.query)('SELECT COUNT(*) as count FROM stores');
        const ratingCountRes = await (0, db_1.query)('SELECT COUNT(*) as count FROM ratings');
        res.status(200).json({
            status: 'success',
            data: {
...
const addUser = async (req, res) => {
    try {
        ...
        const existing = await (0, db_1.query)('SELECT id FROM users WHERE email = ?', [email]);
        if (existing.length > 0) {
            res.status(400).json({ status: 'fail', message: 'Email already registered' });
            return;
        }
        const hashedPassword = await bcryptjs_1.default.hash(password, 10);
        const result = await (0, db_1.query)('INSERT INTO users (name, email, password, address, role) VALUES (?, ?, ?, ?, ?)', [name, email, hashedPassword, address, role]);
        res.status(201).json({
            status: 'success',
...
const addStore = async (req, res) => {
    try {
        ...
        const existingStore = await (0, db_1.query)('SELECT id FROM stores WHERE email = ?', [email]);
        ...
        if (ownerId) {
            const users = await (0, db_1.query)('SELECT role FROM users WHERE id = ?', [ownerId]);
            if (users.length === 0) {
            ...
            const assignedStore = await (0, db_1.query)('SELECT id FROM stores WHERE owner_id = ?', [ownerId]);
            if (assignedStore.length > 0) {
                res.status(400).json({ status: 'fail', message: 'Selected owner already owns another store' });
                return;
            }
        }
        const result = await (0, db_1.query)('INSERT INTO stores (name, email, address, owner_id) VALUES (?, ?, ?, ?)', [name, email, address, ownerId || null]);
        res.status(201).json({
            status: 'success',
...
const getUsers = async (req, res) => {
    ...
    `;
        ...
        const users = await (0, db_1.query)(sql, params);
...
const getAvailableOwners = async (req, res) => {
    ...
    `;
        const owners = await (0, db_1.query)(sql);
...
exports.getAvailableOwners = getAvailableOwners;
...
const deleteStore = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await (0, db_1.query)('DELETE FROM stores WHERE id = ?', [id]);
        if (result.affectedRows === 0) {
...
const updateStore = async (req, res) => {
    try {
        ...
        const store = await (0, db_1.query)('SELECT id, owner_id FROM stores WHERE id = ?', [id]);
        if (store.length === 0) {
            res.status(404).json({ status: 'fail', message: 'Store not found' });
            return;
        }
        // Check if email is already used by another store
        const emailCheck = await (0, db_1.query)('SELECT id FROM stores WHERE email = ? AND id != ?', [email, id]);
        ...
        if (ownerId && ownerId !== store[0].owner_id) {
            ...
            const assignedStore = await (0, db_1.query)('SELECT id FROM stores WHERE owner_id = ? AND id != ?', [ownerId, id]);
            if (assignedStore.length > 0) {
                res.status(400).json({ status: 'fail', message: 'Selected owner already owns another store' });
                return;
            }
        }
        await (0, db_1.query)('UPDATE stores SET name = ?, email = ?, address = ?, owner_id = ? WHERE id = ?', [name, email, address, ownerId || null, id]);
        res.status(200).json({
            status: 'success',
