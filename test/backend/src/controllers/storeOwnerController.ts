...
export const getOwnerDashboard = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
    ...
    const stores = await query('SELECT id, name, email, address FROM stores WHERE owner_id = ?', [ownerId]);
    ...
    const ratings = await query(ratingsSql, [storeId]);
